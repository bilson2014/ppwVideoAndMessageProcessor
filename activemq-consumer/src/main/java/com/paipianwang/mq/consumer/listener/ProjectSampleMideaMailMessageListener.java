...
@Component
public class ProjectSampleMideaMailMessageListener implements SessionAwareMessageListener<Message> {
	private static final Logger logger = LoggerFactory.getLogger(ProjectSampleMideaMailMessageListener.class);
	
	@Autowired
	private JmsTemplate activeMqJmsTemplate;
	@Autowired
	private Destination projectSampleMideaQueue;
	@Autowired
	...
	public void onMessage(Message message, Session session) throws JMSException {
		try {
			...
			try {
				productMailService.sendProjectSampleMIdea(projectId);
				Thread.sleep(200);
			} catch (Exception e) {
				// 发送异常，重新放回队列-最多尝试3次
				if(time<=2){
					time++;
					json.put("time", time);
					activeMqJmsTemplate.send(projectSampleMideaQueue, new MessageCreator() {
