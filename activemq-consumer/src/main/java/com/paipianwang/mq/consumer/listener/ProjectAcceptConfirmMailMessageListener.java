...
@Component
public class ProjectAcceptConfirmMailMessageListener implements SessionAwareMessageListener<Message> {
	private static final Logger logger = LoggerFactory.getLogger(ProjectAcceptConfirmMailMessageListener.class);
	...
	public void onMessage(Message message, Session session) throws JMSException {
		try {
			...
			try {
				productMailService.sendProjectAcceptConfirm(ms);;
				Thread.sleep(200);
			} catch (Exception e) {
				// 发送异常，重新放回队列
