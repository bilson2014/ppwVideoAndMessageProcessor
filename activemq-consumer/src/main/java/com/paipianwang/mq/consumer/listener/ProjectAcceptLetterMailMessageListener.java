...
public class ProjectAcceptLetterMailMessageListener implements SessionAwareMessageListener<Message> {
	...
	@Autowired
	private Destination projectAcceptLetterQueue;
	@Autowired
	...
	public void onMessage(Message message, Session session) throws JMSException {
		try {
			...
			try {
				productMailService.sendProjectAcceptLetter(projectId);;
				Thread.sleep(200);
			} catch (Exception e) {
				// 发送异常，重新放回队列-最多尝试3次
				if(time<=2){
					time++;
					json.put("time", time);
					activeMqJmsTemplate.send(projectAcceptLetterQueue, new MessageCreator() {
